...
impl ApiRequest {
    ...
    pub fn fetch(&mut self, keep_alive: bool) -> Result<Vec<u8>, StrErr> {
        ...
        //                     }
        //                     Err(err) => {
        //                         let message = format!("{}.fetch | write to tcp stream error: {:?}", self.dbgid, err);
        //                         log::warn!("{}", message);
        //                         Err(message.into())
        //                     }
        //                 }
        //             }
        //             Err(err) => {
        //                 let message = format!("{}.fetch | Serialize error: {:?}", self.dbgid, err);
        //                 log::warn!("{}", message);
        //                 Err(message.into())
        //             }
        //         }
    ...
    //     let status = Err(format!("{}.read_all | tcp stream error: {:?}", self.dbgid, err).into());
    //     match err.kind() {
