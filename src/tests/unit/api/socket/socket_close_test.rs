...
mod socket_close {
    ...
    fn tcp_stream_close() {
        ...
        log::debug!("\n{}", dbg);
        let test_duration = TestDuration::new(&dbg, Duration::from_secs(24));
            ...
            Ok(tcp_listener) => {
                ...
                let _ = thread::spawn(move || {
                    match tcp_listener.accept() {
                        Ok((_, addr)) => {
                            log::debug!("{}.server | Connection accepted from: {:?}", dbg, addr);
                        ...
                        Err(err) => {
                            panic!("{}.server | Accept Error: {:?}", dbg, err);
                        },
                    };
                });
                Ok(())
        ...
        log::debug!("{}.server | Bind result: {:?}", dbg, result);
        assert!(result.is_ok(), "step {} \nresult: {:?}\ntarget: {:?}", step.inc(), result.is_ok(), true);
        match TcpStream::connect(addr) {
            Ok(socket) => {
                log::debug!("{} | Connected on: {:?}", dbg, socket.peer_addr());
                ...
                while !exit.load(Ordering::SeqCst) {
                    ...
                    log::debug!("{} | Read done in: {:?}", dbg, time.elapsed());
                    log::debug!("{} | Read result: {:?}", dbg, result);
                    match result {
                        Ok(len) => {
                            if len == 0 {
                                log::debug!("{} | Connection closed", dbg);
            ...
            Ok(_) => {
                ...
                while !exit.load(Ordering::SeqCst) {
                    ...
                    match result {
                        ...
                        Err(err) => {
                            log::debug!("{} | Connection closed, error: {}", dbg, err);
                            exit.store(true, Ordering::SeqCst);
                        }
                    }
                }
